...
public class GameClient implements Runnable {
    ...
    private void readPacket(GamePacket packet) {
        ...
        switch (header) {
            ...
            case "AuctionRequest":
                AuctionRequest aucReq = (AuctionRequest) packet;
                int userid = aucReq.getUserId();
                long itemid = aucReq.getItemId();

                switch (aucReq.getType()) {
                    case BUY:
                        this.server.buyItem(userid, itemid);
                        break;
                    case SELL:
                        this.server.sellItem(userid, itemid);
                        break;
                    case CANCEL:
                        this.server.cancelAuction(userid, itemid);
                        break;
                }
                break;
            default:
